...
class PushTask(bases.Task):
    def __init__(
        ...
        if len(self.obstacles) > 0:
            assert hasattr(
                sensors, sensor), f'Sensor={sensor} not implemented.'
    ...
    def get_observation(self):
        ...
        puck_to_goal = self.puck.get_position(
        )[:2] - self.agent.get_position()[:2]
        delta_xyz = self.goal.get_position() - self.agent.get_position()
        # rescale into [-2, +2]
...
class NavTask(bases.Task):
    def __init__(
        ...
        self.detection_distance = 1
        # depends on how wide obstacles are, rough number
        self.obstacle_obstacle_collision_distance = 0.5
        self.agent_obstacle_distance = 0.5  # reduce agent obstacle spacing
        # reduce distance to objects, especially important for more complex
        # agents such as Ant to enable random exploration to collect sparse
        # rewards
        ...
        # new state for NavTask
        self._accomplished_task = False
        self._accomplished_goal = False

        self._task = None
        self._pos_task = None
        self._goals = None
        self._penalties = None
        self._obs_loc_dict = None

        self._curr_region_symbols = set()

        self._rmax = 2.
        self._rstep = -0.005
        self.multiplier = pow(2./abs(self._rstep), 1./3.)
        self._bad_step = self.multiplier * self._rstep
        self._bad_term = self.multiplier * self._bad_step
        self._worst_term = self.multiplier * self._bad_term

        self._safety_violation = 0  # set to one if there is a safety violation that step

    def set_task_goal(self, task, goals, pos_task=True):
        self._task = set(task)
        if goals is None:
            all_goals = [frozenset(o.name.split('_')) for o in self.obstacles]
            self._goals = list(sorted(all_goals))
        else:
            assert all([isinstance(g, frozenset) for g in goals])
            self._goals = goals
        self._pos_task = pos_task

        # print(f"task: {task}")
        # print(f"_goals: {self._goals}")
        # print(f"_pos_task: {self._pos_task}")

    def set_penalty(self, penalties):
        self._penalties = penalties

    def set_locs(self, obs_loc_dict):
        '''
        Set the obstacle locations using a dictionary
        '''
        self._obs_loc_dict = obs_loc_dict

    def calculate_cost(self):
        """determine costs depending on agent and obstacles. """
        info = {'env': 'NavTask',
                'cost': self._safety_violation,
                'symbols': self._produced_symbols}
        return info

    def calculate_reward(self):
        """ Generate rewards based on task for each goal """
        self._safety_violation = 0
        symbols = self.produce_symbols()
        contains_task = bool(self._task & symbols)
        # print(f"calc reward _task : {self._task}")
        # print(f"calc reward _goals : {self._goals}")
        if not symbols:
            return self._rstep

        if not self._penalties:
            reward_dict = {g: 0. for g in self._goals}
            for goal in self._goals:
                match_goal = symbols == goal
                if not self._pos_task:
                    # negated task
                    contains_task = not contains_task
                # TODO: might need to use done
                assert symbols
                if contains_task and match_goal:
                    reward_dict[goal] = self._rmax
                else:
                    reward_dict[goal] = self._rstep
            return max(reward_dict.values())

        assert self._penalties
        reward_dict = {g: 0. for g in self._goals}
        if self._pos_task:
            for goal in self._goals:
                match_goal = goal == symbols
                if not match_goal:
                    reward_dict[goal] = self._worst_term
                    self._safety_violation = 1
                elif not contains_task:
                    reward_dict[goal] = self._bad_step
                    self._safety_violation = 1
                else:
                    assert match_goal and contains_task
                    reward_dict[goal] = self._rmax
        else:
            for goal in self._goals:
                match_goal = goal == symbols
                if not match_goal and contains_task:
                    reward_dict[goal] = self._worst_term
                    self._safety_violation = 1
                elif not match_goal and not contains_task:
                    reward_dict[goal] = self._worst_term
                    self._safety_violation = 1
                elif match_goal and not contains_task:
                    reward_dict[goal] = self._rmax
                elif match_goal and contains_task:
                    reward_dict[goal] = self._bad_term
                    self._safety_violation = 1
                else:
                    assert False

        reward = max(reward_dict.values())
        return reward

    def produce_symbols(self):
        '''
        Check for symbols -- must be called only once per step due to stutter-free semantics
        Use self._produced_symbols to check produced symbols
        '''
        cols = self.get_collisions()
        if len(cols) > 1:
            raise Warning('SIMULTANEOUS COLLISIONS')
        symbols = set()
        for c in cols:
            self._accomplished_task = bool(c.symbols & self._task)
            self._accomplished_goal = any(
                [c.symbols == g for g in self._goals])
            symbols |= c.symbols

        if symbols == self._curr_region_symbols:
            # only produce symbols upon entering a region
            symbols = set()
        else:
            self._curr_region_symbols = symbols
        self._produced_symbols = symbols
        return symbols

    def equip_agent_with_sensors(self):
        # Pseudo LIDARs for each obstacle
    ...
    def get_object_to_object_collisions(self) -> list:
        """Counts the number of collisions with bombs that occurred after
        the last simulation step call."""
        collisions = []
        for obj1, obj2 in itertools.combinations(self.obstacles, 2):
            # print(f"checking objects: {obj1}, {obj2}")
            # print(f"object1 position: {obj1.get_position()[:2]}")
            # print(f"object2 position: {obj2.get_position()[:2]}")
            xy_diff = obj1.get_position()[:2] - obj2.get_position()[:2]
            dist = np.linalg.norm(xy_diff)
            if dist <= self.obstacle_obstacle_collision_distance:
                print(f"OBJECT-OBJECT COLLISION: {obj1.name}, {obj2.name}")
                collisions.append([obj1.name, obj2.name])

        return collisions

    def get_observation(self):
        # no task specific information in Nav
        return []

    @property
    def goal_achieved(self):
        # Assume we're only done when accomplishing the main task
        # This seemed to work for the original discrete env but we can make this goal-specific as needed
        return self._accomplished_goal
    ...
    def specific_reset(self) -> None:
        ...
        if self._task is None:
            raise RuntimeError(
                'Must set task before resetting env and after calling render (only call render once at the beginning)')
        if self._goals is None:
            raise RuntimeError(
                'Must set goals before resetting env and after calling render (only call render once at the beginning)')
        if self._penalties is None:
            raise RuntimeError(
                'Must set penalty before resetting env and after calling render (only call render once at the beginning)')
    ...
    def set_obstacles(self):
        if len(self.obstacles) > 0:
            if self._obs_loc_dict is None:
                ...
                for ob in self.obstacles:
                    ob.set_position(self._obs_loc_dict[ob.name])
